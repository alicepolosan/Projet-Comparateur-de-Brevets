...
def liste_annees (brevet_list):
    dic=brevet_list[-1]
    annee_depot=dic['annee_depot']
    annee_delivrance=dic['annee_delivrance']
    annee_fin= annee_depot + 20
    year_list= json.dumps(list(np.arange(annee_delivrance,annee_fin)))
    return year_list
...
def couts_année (brevet_list):
    ...
    data=data[indices]
    sommes= data.sum()
    couts_BE= json.dumps(list(sommes.to_numpy()))
    couts_BU= json.dumps(list(np.sum(couts_BU_annees)))
...
def couts_cumulés (brevet_list):
    ...
    data=data[indices]
    sommes= data.sum()
    couts_BE=json.dumps(list(np.cumsum(sommes.to_numpy())))
    couts_BU=json.dumps(list(np.cumsum(np.array(couts_BU_annees))))
    return couts_BE, couts_BU

def total_calcul (brevet_list):
